...
export class EmployeesRepository {
  ...
  async findByIdRaw(id: number): Promise<EmployeeModel | null> {
  ...
  async update(id: number, data: any): Promise<EmployeeModel> {
    const result = await pool.query(
      `UPDATE employees SET document_type = $1, name = $2, address = $3, phone = $4, email = $5, module = $6
  ...
  async delete(id: number): Promise<void> {
    await pool.query('UPDATE employees SET active = false WHERE id = $1', [id])
  }

  async findByDocument(document: string): Promise<EmployeeModel | null> {
    const result = await pool.query('SELECT * FROM employees WHERE document = $1', [document])
    return result.rows[0] || null
  }

  async activate(id: number): Promise<void> {
    await pool.query('UPDATE employees SET active = true WHERE id = $1', [id])
  }
}
