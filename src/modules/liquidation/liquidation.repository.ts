...
export class LiquidationRepository {
    ...
    async createManyLiquidationDeatails(details: any[], client: QueryClient = pool): Promise<void>{
        const values: any[] = []
        ...
        await client.query(
            `INSERT INTO liquidation_details(
                liquidation_id,
                employee_id,
                module,
                work_date,
    ...
    async getAllLiquidationDeatails (): Promise<LiquidationDetailsModel[]>{
        const result = await pool.query(
            `SELECT * FROM liquidation_details`
        )
        return result.rows
    }
    async getLiquidationById(id: number): Promise<LiquidationModel>{
        const result = await pool.query(
            `SELECT * FROM liquidations
            WHERE id=$1`,
            [id]
    ...
    async getLiquidationDeatailsById(liquidation_id: number): Promise<LiquidationDetailsModel[]>{
        const result = await pool.query(
            `SELECT * FROM liquidation_details
