import { LiquidationRepository } from "./liquidation.repository";
import { CreateLiquidationDetailsDto, CreateLiquidationDto } from "./liquidation.Dto";
import { withTransaction } from "../../shared/db/transaction";
import { PoolClient } from "pg";
import { ProductionRepository } from "../production/production.repository";
import { workLogsRepository } from "../work-logs/work-logs.repository";

export class liquidationService {
    constructor(private repo = new LiquidationRepository(),
        private repoProduction = new ProductionRepository(),
        private repoWorkLogs = new workLogsRepository()
    ) { }

    async create(data: CreateLiquidationDto) {
        return await withTransaction(async (client: PoolClient) => {

            const liquidation = await this.repo.createLiquidation(data, client);
            console.log(liquidation);

            const rules = await this.repo.getAllIncentiveRules();
            console.log(rules);

            const incentiveMap = new Map(
                rules.map(r => [r.efficiency_point, Number(r.value)])
            );

            const workLogs = await this.repoWorkLogs.findByModuleAndDateRange(
                data.module,
                String(data.start_date),
                String(data.end_date)
            );
            console.log(workLogs);

            const production = await this.repoProduction.getSumaryByModuleAndDateRange(
                data.module,
                String(data.start_date),
                String(data.end_date)
            );
            console.log(production);

            const formatDate = (date: Date) => {
                return new Date(date).toISOString().split("T")[0];
            };

            const productionMap = new Map<string, number>();

            for (const p of production) {
                const key = formatDate(p.work_date);
                productionMap.set(key, Number(p.produced_minutes));
            }

            const workLogsByDate = new Map<string, any[]>();

            for (const w of workLogs) {
                const date = formatDate(w.work_date);

                if (!workLogsByDate.has(date)) {
                    workLogsByDate.set(date, []);
                }

                workLogsByDate.get(date)!.push(w);
            }

            const details: any[] = [];

            for (const [date, logs] of workLogsByDate.entries()) {

                const produced = productionMap.get(date) || 0;

                const totalWorked = logs.reduce((sum, l) => sum + l.minutes_worked, 0);
                const totalDowntime = logs.reduce((sum, l) => sum + l.minutes_downtime, 0);

                const totalEffective = totalWorked - totalDowntime;

                let efficiency = 0;

                if (totalEffective > 0) {
                    efficiency = (produced / totalEffective) * 100;
                }

                const effPoint = Math.min(Math.floor(efficiency), 105);
                const incentiveBase = incentiveMap.get(effPoint) || 0;

                for (const w of logs) {

                    const worked = w.minutes_worked;
                    const downtime = w.minutes_downtime;

                    const effectiveEmployee = worked - downtime;

                    const participation = totalEffective > 0
                        ? effectiveEmployee / totalEffective
                        : 0;

                    const payment = incentiveBase * participation;

                    details.push({
                        liquidation_id: liquidation.id,
                        employee_id: w.employee_id,
                        module: data.module,
                        work_date: date,

                        worked_minutes: worked,
                        downtime_minutes: downtime,

                        produced_minutes: produced,
                        efficiency: Number(efficiency.toFixed(2)),

                        incentive_base: incentiveBase,
                        payment: Number(payment.toFixed(2))
                    });
                }
            }

            await this.repo.createManyLiquidationDeatails(details, client);

            return {
                liquidation,
                total_records: details.length
            };
        });
    }
    async findLiquidation() {
        const result = await this.repo.getAllLiquidation()
        return result;
    }
    async findLiquidationDetails(id: number) {
        const result = await this.repo.getLiquidationDeatailsById(id);
        return result;
    }
}